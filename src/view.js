class Country {
  ...
  addHandlerButton(handler) {
    document.querySelectorAll(".border-button").forEach((el) =>
      el.addEventListener("click", (e) => {
    ...
    const html = `
        ...
        <div class="country">
            ...
            <div class="country__details">
                ...
                <p class="country-label">Population: <span class="country-data population">${
                  this.data.population
                }</span></p>
                <p class="country-label">Region: <span class="country-data region">${
                  this.data.region
                }</span></p>
                <p class="country-label">Sub Region: <span class="country-data capital">${
                  this.data.subregion
                }</span></p>
                <p class="country-label">Capital: <span class="country-data capital">${
                  this.data.capital
                }</span></p>
            </div>
            <div class="country__secondary-details">
                <p class="country-label">Top Level Domain: <span class="country-data capital">${
                  this.data.tld[0]
                }</span></p>
            ...
            <div class="borders">
                ${
                  !this.data.borders
                    ? `sorry no borders`
                    : borders
                        .map((el) => this.renderBorderButton(el.name.common))
                        .join("")
                }               
            </div>
        </div>
